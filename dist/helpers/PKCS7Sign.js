...
class PKCS7Sign extends Object {
  extractSignature(pdf) {
    ...
    const signedData = Buffer.concat([pdf.slice(parseInt(byteRangeArr[0]), parseInt(byteRangeArr[1])), pdf.slice(parseInt(byteRangeArr[2]), parseInt(byteRangeArr[2]) + parseInt(byteRangeArr[3]))]);
    let signatureHex = pdf.slice(parseInt(byteRangeArr[0]) + (parseInt(byteRangeArr[1]) + 1), parseInt(byteRangeArr[2]) - 1).toString("binary");
  ...
  async signPDFBuffer(pdfBuffer, certificate, passphrase, pkcsSigner) {
    ...
    if (pkcsSigner) {
      signedPdfBuffer = await signObj.signWithPkcs11(pdfBuffer, pkcsSigner, passphrase);
    } else {
      const p12Buffer = new Buffer(certificate, "base64");
      signedPdfBuffer = signObj.sign(pdfBuffer, p12Buffer, {
  ...
  async prepareSignPDFDoc(pdfDoc, signatureImageBytes, x, y, w, h) {
    ...
    pages[0].node.set(PDFName.of('Annots'), pdfDoc.context.obj([widgetDictRef])); // Create an AcroForm object containing our signature widget
    ...
    sig.acroField.getWidgets().forEach(widget => {
      ...
      const appearance = [...drawRectangle({
        x: 0,
        y: 0,
        width,
        height,
